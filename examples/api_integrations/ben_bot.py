...
class MyClient(fortnitepy.Client):
    ...
    async def fetch_cosmetic_id(self, display_name):
        async with self.session.get(
            BEN_BOT_BASE + "/api/cosmetics/search", params={"displayName": display_name}
        ) as r:
            data = await r.json()
            return data
    ...
    async def event_party_message(self, message):
        ...
        if command == "!setoutfit":
            ...
            await self.party.me.set_outfit(asset=cid)
        ...
        # runs the emote specified for 10 seconds
            ...
            await self.party.me.set_emote(asset=eid, run_for=10)
