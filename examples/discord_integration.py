...
discord_bot = discord_commands.Bot(
    command_prefix="!", description=description, case_insensitive=True
)
...
async def on_message(message):
    ...
    print(
        'Received discord message from {0.author.display_name} | Content "{0.content}"'.format(
            message
        )
    )
...
async def event_friend_message(message):
    print(
        'Received fortnite message from {0.author.display_name} | Content "{0.content}"'.format(
            message
        )
    )
